...
try {
    ...
    Write-Host "✓ SSH is available: $sshVersion" -ForegroundColor Green
} catch {
...
try {
    ...
    $result = ssh -o ConnectTimeout=10 -o BatchMode=no "$Username@$Server" "echo 'SSH connection successful!' && whoami && pwd" 2>&1
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  1. Set up SSH keys for passwordless login" -ForegroundColor White
